...
const ExamManagerDashboard = () => {
  ...
  const stats = [
    { label: "Đề đang mở", value: openExams, icon: FileText, color: "bg-blue-500/10 text-blue-600" },
    { label: "Tổng lượt thi", value: totalAttempts.toLocaleString("vi-VN"), icon: Users, color: "bg-emerald-500/10 text-emerald-600" },
    { label: "Doanh thu", value: `${(totalRevenue / 1000000).toFixed(1)}M`, icon: DollarSign, color: "bg-amber-500/10 text-amber-600" },
              ...
              return (
                <div key={a.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-xl">
                  ...
                  <div className="text-right">
                    <p className={`text-sm font-bold ${a.score >= 5 ? "text-emerald-600" : "text-red-600"}`}>{a.score}/{a.totalQuestions > 10 ? 10 : a.totalQuestions}</p>
            ...
            {exams.filter(e => e.status === "open").sort((a, b) => b.attempts - a.attempts).slice(0, 5).map((e, i) => (
              <div key={e.id} className="flex items-center justify-between p-2">
                <div className="flex items-center gap-2">
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${i === 0 ? "bg-amber-500/20 text-amber-600" : i === 1 ? "bg-gray-300/30 text-gray-600" : "bg-muted text-muted-foreground"}`}>{i + 1}</span>
