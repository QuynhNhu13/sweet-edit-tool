...
const ExamManagerExams = () => {
  ...
  const [form, setForm] = useState({ name: "", subject: "Toán", duration: "90", questionCount: "50", fee: "10000", year: "2025", status: "draft" as ExamStatus, aiMode: "auto_generate" as AIMode, difficulty: "medium" as Difficulty, aiProctoring: true });
  ...
  const handleCreate = () => {
    ...
    addExam({
      ...
      aiProctoring: form.aiProctoring, attempts: 0, revenue: 0, completionRate: 0, aboveAverageRate: 0, createdAt: "03/03/2026",
    });
    setForm({ name: "", subject: "Toán", duration: "90", questionCount: "50", fee: "10000", year: "2025", status: "draft", aiMode: "auto_generate", difficulty: "medium", aiProctoring: true });
    setShowCreate(false);
  ...
  const handleEdit = () => {
    if (!editExam) return;
    updateExam(editExam.id, { name: form.name, subject: form.subject, duration: parseInt(form.duration), questionCount: parseInt(form.questionCount), fee: parseInt(form.fee), year: parseInt(form.year), status: form.status, aiMode: form.aiMode, difficulty: form.difficulty, aiProctoring: form.aiProctoring });
    setEditExam(null);
    toast({ title: "Cập nhật đề thi thành công" });
  };

  const openEdit = (e: Exam) => {
    setForm({ name: e.name, subject: e.subject, duration: String(e.duration), questionCount: String(e.questionCount), fee: String(e.fee), year: String(e.year), status: e.status, aiMode: e.aiMode, difficulty: e.difficulty, aiProctoring: e.aiProctoring });
    setEditExam(e);
  };
      ...
      <div className="grid grid-cols-3 gap-3">
        <div><Label>Trạng thái</Label>
          <Select value={form.status} onValueChange={v => setForm(p => ({ ...p, status: v as ExamStatus }))}>
            <SelectTrigger className="rounded-xl mt-1"><SelectValue /></SelectTrigger>
            <SelectContent><SelectItem value="draft">Nháp</SelectItem><SelectItem value="open">Đang mở</SelectItem><SelectItem value="closed">Đã đóng</SelectItem></SelectContent>
          </Select>
        </div>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            ...
            <SelectContent><SelectItem value="all">Tất cả</SelectItem><SelectItem value="draft">Nháp</SelectItem><SelectItem value="open">Đang mở</SelectItem><SelectItem value="closed">Đã đóng</SelectItem></SelectContent>
          </Select>
        </div>
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          ...
          <DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Tạo đề thi mới</DialogTitle></DialogHeader>{formFields}<Button onClick={handleCreate} className="w-full rounded-xl mt-2">Tạo đề thi</Button></DialogContent>
        </Dialog>
      </div>
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {filtered.map(e => (
                <TableRow key={e.id}>
                  <TableCell><p className="font-medium text-foreground text-sm">{e.name}</p><p className="text-[10px] text-muted-foreground">{e.createdAt}</p></TableCell>
                  ...
                  <TableCell className="text-sm font-medium">{e.attempts.toLocaleString("vi-VN")}</TableCell>
                  <TableCell><Badge variant={statusConfig[e.status].variant}>{statusConfig[e.status].label}</Badge></TableCell>
                  <TableCell>
      ...
      <Dialog open={!!editExam} onOpenChange={() => setEditExam(null)}>
        <DialogContent className="max-w-lg"><DialogHeader><DialogTitle>Chỉnh sửa đề thi</DialogTitle></DialogHeader>{formFields}<Button onClick={handleEdit} className="w-full rounded-xl mt-2">Lưu thay đổi</Button></DialogContent>
      </Dialog>
