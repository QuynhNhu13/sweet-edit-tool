    ...
    <div className="flex h-screen bg-muted/30 overflow-hidden">
      ...
      <aside className={cn("sidebar-theme mx-3 my-3 rounded-[1.75rem] border border-sidebar-border/70 bg-card shadow-elevated backdrop-blur-sm flex flex-col shrink-0 transition-all duration-300", collapsed ? "w-[72px]" : "w-[260px]")}>
        <div className="h-16 flex items-center gap-3 px-4 border-b border-sidebar-border/40">
          ...
          {!collapsed && (
            <div className="min-w-0">
                ...
                <p className="text-[11px] text-sidebar-foreground/70 leading-tight">Admin Panel</p>
            </div>
          )}
          <button
        ...
        <nav className="flex-1 px-3 py-5 overflow-y-auto">
          {!collapsed && <p className="text-[10px] font-semibold text-sidebar-muted-foreground uppercase tracking-[0.18em] px-3 mb-3">Menu</p>}
          {navItems.map(item => (
            <NavLink
              key={item.to}
              to={item.to}
              ...
              className={({ isActive }) =>
                cn(
                  "flex items-center gap-3 rounded-full text-[13px] font-semibold transition-all duration-300 group relative",
